...
% path_folder = strcat(path_save, subject_id, '/fitting_x108-123y124-139z37-42/');
...
% load(strcat(path_folder,'space_indexx108-123y124-139z37-42.mat'));

savepath = strcat(path_folder,'space_indexx108-123y124-139z37-42/');
% savepath = strcat(path_folder,'/fitting_x108-123y124-139z37-42/');
...
fod_sele_SN_all_use = fod_sele_SN_RSSdiff(x_subr,y_subr,z_subr,:);
...
for k=1:nn3
    for j=1:nn2
        for i=1:nn1
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%
            [~, ~, ~, ~, ~, peak_pos_SN_final_RSSdiff] = FOD_peak(fod_sele_SN_all_use(i,j,k,:), Dis, kmin, peak_thresh, pos_p, theta_p, phi_p);
