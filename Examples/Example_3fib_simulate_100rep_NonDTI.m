%% Example script for FOD with 3 fiber 
...
b = [1, 1, 1]; % back ground magnetic field strength (1 is same as b=1000)
...
fod2_s = [1/2 0  sqrt(3)/2]; %%1,0,0; x-axis
fod3_s = [1/2 sqrt(2/3) sqrt(1/12)];
...
% save_path = strcat(path_save,'simulation_review/','3fib_sep',num2str(round(sep*180/pi,0)),'_lmax',num2str(lmax),'_b',num2str(b(1)),'_ratio',num2str(ratio(1)),'_n',num2str(n_sample),'_sig',num2str(sigma),'/');
save_path = strcat(path_save,'simulation_review/','3fib_sep',num2str(round(sep*180/pi,0)),...
    '_lmax',num2str(lmax),'_b',num2str(b(1)),'_ratio',num2str(ratio(1)),'_n',num2str(n_sample),...
    '_sig',num2str(sigma),'_SH',num2str(DTI_lmax),'/');
...
for rep = 1:n_rep
    ...
    seed = rep*10+21;
    [DWI, theta_r, phi_r, rotationMatrix]= DWI_generate_FOD(J_use, DTI_lmax,b, ratio, weight, theta0, phi0, sigma, half, Rotate, seed);
    ...
    save_name = strcat('3fib_sep',num2str(round(sep*180/pi,0)),'_lmax',num2str(lmax),'_b',num2str(b(1)),'_ratio',num2str(ratio(1)),'_n',num2str(n_sample),'_sig',num2str(sigma),'_rep',num2str(rep),'.mat');
