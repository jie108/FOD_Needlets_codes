...
folder_name = paste0('3fib_sep',toString(sep),'_lmax',toString(lmax),'_b',toString(b),
                     '_ratio',toString(ratio),'_n',toString(n_sample),'_sig',toString(sigma),'_SH',toString(DTI_lmax),'/')
# folder_name = paste0('3fib_sep',toString(sep),'_lmax',toString(lmax),'_b',toString(b),
...
for(rep in 1:N_rep){
  ...
  out <- try(biased.mle.cs2.iso(dwis=c(temp$DWI), design=design_temp,
                                ...
                                display=T, pre.thres=opt$pre.thres,
  ...
  if(out$n.fiber == 3){
    ...
    fib1_true = c(sin(temp$theta.r[1])*cos(temp$phi.r[1]),sin(temp$theta.r[1])*sin(temp$phi.r[1]),
                  cos(temp$theta.r[1]))
    fib2_true = c(sin(temp$theta.r[2])*cos(temp$phi.r[2]),sin(temp$theta.r[2])*sin(temp$phi.r[2]),
                  cos(temp$theta.r[2]))
    fib3_true = c(sin(temp$theta.r[3])*cos(temp$phi.r[3]),sin(temp$theta.r[3])*sin(temp$phi.r[3]),
                  cos(temp$theta.r[3]))
    ...
    sep1_DiST[rep] = min(acos(sum(out$vs[1,]*out$vs[2,])/(sqrt(sum(out$vs[1,]*out$vs[1,]))*sqrt(sum(out$vs[2,]*out$vs[2,])))),
                         abs(pi-acos(sum(out$vs[1,]*out$vs[2,])/(sqrt(sum(out$vs[1,]*out$vs[1,]))*sqrt(sum(out$vs[2,]*out$vs[2,]))))))*180/pi
    sep2_DiST[rep] = min(acos(sum(out$vs[1,]*out$vs[3,])/(sqrt(sum(out$vs[1,]*out$vs[1,]))*sqrt(sum(out$vs[3,]*out$vs[3,])))),
                         abs(pi-acos(sum(out$vs[1,]*out$vs[3,])/(sqrt(sum(out$vs[1,]*out$vs[1,]))*sqrt(sum(out$vs[3,]*out$vs[3,]))))))*180/pi
    sep3_DiST[rep] = min(acos(sum(out$vs[3,]*out$vs[2,])/(sqrt(sum(out$vs[3,]*out$vs[3,]))*sqrt(sum(out$vs[2,]*out$vs[2,])))),
                         abs(pi-acos(sum(out$vs[3,]*out$vs[2,])/(sqrt(sum(out$vs[3,]*out$vs[3,]))*sqrt(sum(out$vs[2,]*out$vs[2,]))))))*180/pi
...
save(nfib_DiST, angle1_DiST, angle2_DiST, angle3_DiST, sep1_DiST, sep2_DiST, sep3_DiST, theta1_DiST, theta2_DiST,
     ...
     correct_DiST, over_DiST, under_DiST, angle1_DiST_mean, angle2_DiST_mean, angle3_DiST_mean,
     angle1_DiST_sd, angle2_DiST_sd, angle3_DiST_sd, sep1_mean, sep2_mean, sep3_mean,
...
writeMat(mat_file, nfib_DiST=nfib_DiST, angle1_DiST=angle1_DiST, 
         ...
         angle1_DiST_mean=angle1_DiST_mean, angle2_DiST_mean=angle2_DiST_mean, 
         angle3_DiST_mean=angle3_DiST_mean, angle1_DiST_sd=angle1_DiST_sd,
         angle2_DiST_sd=angle2_DiST_sd, angle3_DiST_sd=angle3_DiST_sd, 
         sep1_mean=sep1_mean, sep2_mean=sep2_mean, sep3_mean=sep3_mean)
