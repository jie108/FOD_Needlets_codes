% Analysis of 3fiber simulation with 100 replicates
...
b = [3,3,3]; % back ground magnetic field strength (1 is same as b=1000)
ratio = [10,10,10]; % ratio of the leading eigenvalue to the second eigenvalue in the signal simulation
weight = [0.3,0.3,0.4];
...
fod1_s = [1 0 0]; %%0,0,1; z-[0 0 1] [ sqrt(3)/2 0  1/2] [0.7071 0  0.7071] [1/2 0  sqrt(3)/2] [0.2588 0   0.9659] [sqrt(3)/2 0 1/2] [0.7660 0  0.6428]
fod2_s = [0.2588 0   0.9659]; %%1,0,0; x-axis
fod3_s = [0.2588 0.9453 0.1986];
sep=acos(fod1_s*fod2_s');

%%% theta, phi of the fibers
phi0 = atan2([fod1_s(2) fod2_s(2) fod3_s(2)],[fod1_s(1) fod2_s(1) fod3_s(1)]);   %%phi: azimuthal  angle, [0,2pi)
phi0 = phi0+(phi0<0)*2*pi;
theta0 = acos([fod1_s(3) fod2_s(3) fod3_s(3)]);             %% theta: polar angle, [0,pi)
%fod1_s = [0.7071 0  0.7071]; %%0,0,1; z-[0 0 1] [ sqrt(3)/2 0  1/2] [0.7071 0  0.7071] [1/2 0  sqrt(3)/2] [0.2588 0   0.9659]
%fod2_s = [1 0 0]; %%1,0,0; x-axis
%fod3_s = [0.7071 0.6436 0.2929]; %%[1/2 sqrt(2/3) sqrt(1/12)], [0.2588 0.9453 0.1986] [0 1 0] [0.7071 0.6436 0.2929]
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
coe_sh3_lmax8 = Dirac_SH_coe(lmax8,theta0(3),phi0(3));
dirac_sh1_lmax8 = SH_J5_all_lmax8*coe_sh1_lmax8;
dirac_sh2_lmax8 = SH_J5_all_lmax8*coe_sh2_lmax8;
dirac_sh3_lmax8 = SH_J5_all_lmax8*coe_sh3_lmax8;
dirac_sh_lmax8 = weight(1)*dirac_sh1_lmax8+weight(2)*dirac_sh2_lmax8+weight(3)*dirac_sh3_lmax8;  %%SH representation
...
coe_sh3_lmax12 = Dirac_SH_coe(lmax12,theta0(3),phi0(3));
dirac_sh1_lmax12 = SH_J5_all_lmax12*coe_sh1_lmax12;
dirac_sh2_lmax12 = SH_J5_all_lmax12*coe_sh2_lmax12;
dirac_sh3_lmax12 = SH_J5_all_lmax12*coe_sh3_lmax12;
dirac_sh_lmax12 = weight(1)*dirac_sh1_lmax12+weight(2)*dirac_sh2_lmax12+weight(3)*dirac_sh3_lmax12;  %%SH representation
...
coe_sh3_lmax16 = Dirac_SH_coe(lmax16,theta0(3),phi0(3));
dirac_sh1_lmax16 = SH_J5_all_lmax16*coe_sh1_lmax16;
dirac_sh2_lmax16 = SH_J5_all_lmax16*coe_sh2_lmax16;
dirac_sh3_lmax16 = SH_J5_all_lmax16*coe_sh3_lmax16;
dirac_sh_lmax16 = weight(1)*dirac_sh1_lmax16+weight(2)*dirac_sh2_lmax16+weight(3)*dirac_sh3_lmax16;  %%SH representation
...
angle3_SH_lmax8 = zeros(N_rep,1);
angle3_sCSD_lmax8 = zeros(N_rep,1);
angle3_sCSD_lmax12 = zeros(N_rep,1);
angle3_sCSD_lmax16 = zeros(N_rep,1);
angle3_SN = zeros(N_rep,1);
...
theta3_SH8 = repmat(-99,N_rep,1);
phi3_SH8 = repmat(-99,N_rep,1);

theta3_sCSD8 = repmat(-99,N_rep,1);
phi3_sCSD8 = repmat(-99,N_rep,1);

theta3_sCSD12 = repmat(-99,N_rep,1);
phi3_sCSD12 = repmat(-99,N_rep,1);

theta3_sCSD16 = repmat(-99,N_rep,1);
phi3_sCSD16 = repmat(-99,N_rep,1);

theta3_SN = repmat(-99,N_rep,1);
phi3_SN = repmat(-99,N_rep,1);


sep1_SH_lmax8 = zeros(N_rep,1);
sep1_sCSD_lmax8 = zeros(N_rep,1);
sep1_sCSD_lmax12 = zeros(N_rep,1);
sep1_sCSD_lmax16 = zeros(N_rep,1);
sep1_SN = zeros(N_rep,1);

sep2_SH_lmax8 = zeros(N_rep,1);
sep2_sCSD_lmax8 = zeros(N_rep,1);
sep2_sCSD_lmax12 = zeros(N_rep,1);
sep2_sCSD_lmax16 = zeros(N_rep,1);
sep2_SN = zeros(N_rep,1);

sep3_SH_lmax8 = zeros(N_rep,1);
sep3_sCSD_lmax8 = zeros(N_rep,1);
sep3_sCSD_lmax12 = zeros(N_rep,1);
sep3_sCSD_lmax16 = zeros(N_rep,1);
sep3_SN = zeros(N_rep,1);
...
for rep = 1:N_rep
    ...
    folder_path = strcat(path_save,'simulation_review/','3fib_sep',num2str(round(sep*180/pi,0)),'_lmax',num2str(lmax),'_b',num2str(b(1)),'_ratio',num2str(ratio(1)),'_n',...
        num2str(n_sample),'_sig',num2str(sigma),'_SH',num2str(DTI_lmax),'/');
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if(nfib_SH_lmax8(rep)==3)
        fod_temp = [fod1_s; fod2_s; fod3_s];
        id_temp = [1 2 3];
        ...
        ang31 = atan2(norm(cross(fod3_s,peak_pos_SH_final_lmax8(:,1))), dot(fod3_s,peak_pos_SH_final_lmax8(:,1)));
        ang32 = abs(pi - atan2(norm(cross(fod3_s,peak_pos_SH_final_lmax8(:,1))), dot(fod3_s,peak_pos_SH_final_lmax8(:,1))));
        ang333 = min(ang31,ang32);
        
        [angle1_SH_lmax8(rep,1), index_temp] = min([ang111, ang222, ang333]);
        %%%%%
        ...
        elseif(index_temp == 2)
            ...
            phi3_SH8(rep) = phi_temp;
            theta3_SH8(rep) = theta_temp;
        end
        %%%%%
        ...
        ang21 = atan2(norm(cross(fod_temp(2,:),peak_pos_SH_final_lmax8(:,2))), dot(fod_temp(2,:),peak_pos_SH_final_lmax8(:,2)));
        ang22 = abs(pi - atan2(norm(cross(fod_temp(2,:),peak_pos_SH_final_lmax8(:,2))), dot(fod_temp(2,:),peak_pos_SH_final_lmax8(:,2))));
        ang222 = min(ang21,ang22);
        
        [angle2_SH_lmax8(rep,1), index_temp] = min([ang111, ang222]);
        
        %%%%%
        if(dot(fod_temp(index_temp,:)',peak_pos_SH_final_lmax8(:,2))<0)
        ...
        if(id_temp(index_temp) == 1)
            phi1_SH8(rep) = phi_temp;
            theta1_SH8(rep) = theta_temp;
        elseif(id_temp(index_temp) == 2)
            phi2_SH8(rep) = phi_temp;
            theta2_SH8(rep) = theta_temp;
        else
            phi3_SH8(rep) = phi_temp;
        ...
        ang11 = atan2(norm(cross(fod_temp(1,:),peak_pos_SH_final_lmax8(:,3))), dot(fod_temp(1,:),peak_pos_SH_final_lmax8(:,3)));
        ang12 = abs(pi - atan2(norm(cross(fod_temp(1,:),peak_pos_SH_final_lmax8(:,3))), dot(fod_temp(1,:),peak_pos_SH_final_lmax8(:,3))));
        ang111 = min(ang11,ang12);
        angle3_SH_lmax8(rep,1) = ang111;
        if(id_temp(1) == 1)
            phi1_SH8(rep) = phi_temp;
            theta1_SH8(rep) = theta_temp;
        elseif(id_temp(1) == 2)
        ...
        sep1_SH_lmax8(rep,1) = min(ang11, ang12);
        
        ang11 = atan2(norm(cross(peak_pos_SH_final_lmax8(:,1),peak_pos_SH_final_lmax8(:,3))), dot(peak_pos_SH_final_lmax8(:,1),peak_pos_SH_final_lmax8(:,3)));
        ang12 = abs(pi - atan2(norm(cross(peak_pos_SH_final_lmax8(:,1),peak_pos_SH_final_lmax8(:,3))), dot(peak_pos_SH_final_lmax8(:,1),peak_pos_SH_final_lmax8(:,3))));
        sep2_SH_lmax8(rep,1) = min(ang11, ang12);
        
        ang11 = atan2(norm(cross(peak_pos_SH_final_lmax8(:,3),peak_pos_SH_final_lmax8(:,2))), dot(peak_pos_SH_final_lmax8(:,3),peak_pos_SH_final_lmax8(:,2)));
        ang12 = abs(pi - atan2(norm(cross(peak_pos_SH_final_lmax8(:,3),peak_pos_SH_final_lmax8(:,2))), dot(peak_pos_SH_final_lmax8(:,3),peak_pos_SH_final_lmax8(:,2))));
        sep3_SH_lmax8(rep,1) = min(ang11, ang12);
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if(nfib_sCSD_lmax8(rep)==3)
        ...
        ang31 = atan2(norm(cross(fod3_s,peak_pos_sCSD_final_lmax8(:,1))), dot(fod3_s,peak_pos_sCSD_final_lmax8(:,1)));
        ang32 = abs(pi - atan2(norm(cross(fod3_s,peak_pos_sCSD_final_lmax8(:,1))), dot(fod3_s,peak_pos_sCSD_final_lmax8(:,1))));
        ang333 = min(ang31,ang32);
        
        [angle1_sCSD_lmax8(rep,1), index_temp] = min([ang111, ang222, ang333]);
        %%%%%
        ...
        elseif(index_temp == 2)
            ...
            phi3_sCSD8(rep) = phi_temp;
            theta3_sCSD8(rep) = theta_temp;
        end
        %%%%%
        ...
        ang21 = atan2(norm(cross(fod_temp(2,:),peak_pos_sCSD_final_lmax8(:,2))), dot(fod_temp(2,:),peak_pos_sCSD_final_lmax8(:,2)));
        ang22 = abs(pi - atan2(norm(cross(fod_temp(2,:),peak_pos_sCSD_final_lmax8(:,2))), dot(fod_temp(2,:),peak_pos_sCSD_final_lmax8(:,2))));
        ang222 = min(ang21,ang22);
        
        [angle2_sCSD_lmax8(rep,1), index_temp] = min([ang111, ang222]);
        
        %%%%%
        if(dot(fod_temp(index_temp,:)',peak_pos_sCSD_final_lmax8(:,2))<0)
        ...
        ang11 = atan2(norm(cross(fod_temp(1,:),peak_pos_sCSD_final_lmax8(:,3))), dot(fod_temp(1,:),peak_pos_sCSD_final_lmax8(:,3)));
        ang12 = abs(pi - atan2(norm(cross(fod_temp(1,:),peak_pos_sCSD_final_lmax8(:,3))), dot(fod_temp(1,:),peak_pos_sCSD_final_lmax8(:,3))));
        ang111 = min(ang11,ang12);
        angle3_sCSD_lmax8(rep,1) = ang111;
        if(id_temp(1) == 1)
            phi1_sCSD8(rep) = phi_temp;
            theta1_sCSD8(rep) = theta_temp;
        elseif(id_temp(1) == 2)
        ...
        sep1_sCSD_lmax8(rep,1) = min(ang11, ang12);
        
        ang11 = atan2(norm(cross(peak_pos_sCSD_final_lmax8(:,1),peak_pos_sCSD_final_lmax8(:,3))), dot(peak_pos_sCSD_final_lmax8(:,1),peak_pos_sCSD_final_lmax8(:,3)));
        ang12 = abs(pi - atan2(norm(cross(peak_pos_sCSD_final_lmax8(:,1),peak_pos_sCSD_final_lmax8(:,3))), dot(peak_pos_sCSD_final_lmax8(:,1),peak_pos_sCSD_final_lmax8(:,3))));
        sep2_sCSD_lmax8(rep,1) = min(ang11, ang12);
        
        ang11 = atan2(norm(cross(peak_pos_sCSD_final_lmax8(:,3),peak_pos_sCSD_final_lmax8(:,2))), dot(peak_pos_sCSD_final_lmax8(:,3),peak_pos_sCSD_final_lmax8(:,2)));
        ang12 = abs(pi - atan2(norm(cross(peak_pos_sCSD_final_lmax8(:,3),peak_pos_sCSD_final_lmax8(:,2))), dot(peak_pos_sCSD_final_lmax8(:,3),peak_pos_sCSD_final_lmax8(:,2))));
        sep3_sCSD_lmax8(rep,1) = min(ang11, ang12);
    end
    ...
    if(nfib_sCSD_lmax12(rep)==3)
        ...
        ang31 = atan2(norm(cross(fod3_s,peak_pos_sCSD_final_lmax12(:,1))), dot(fod3_s,peak_pos_sCSD_final_lmax12(:,1)));
        ang32 = abs(pi - atan2(norm(cross(fod3_s,peak_pos_sCSD_final_lmax12(:,1))), dot(fod3_s,peak_pos_sCSD_final_lmax12(:,1))));
        ang333 = min(ang31,ang32);
        
        [angle1_sCSD_lmax12(rep,1), index_temp] = min([ang111, ang222, ang333]);
        %%%%%
        ...
        elseif(index_temp == 2)
            ...
            phi3_sCSD12(rep) = phi_temp;
            theta3_sCSD12(rep) = theta_temp;
        end
        %%%%%
        ...
        ang21 = atan2(norm(cross(fod_temp(2,:),peak_pos_sCSD_final_lmax12(:,2))), dot(fod_temp(2,:),peak_pos_sCSD_final_lmax12(:,2)));
        ang22 = abs(pi - atan2(norm(cross(fod_temp(2,:),peak_pos_sCSD_final_lmax12(:,2))), dot(fod_temp(2,:),peak_pos_sCSD_final_lmax12(:,2))));
        ang222 = min(ang21,ang22);
        
        [angle2_sCSD_lmax12(rep,1), index_temp] = min([ang111, ang222]);
        
        %%%%%
        if(dot(fod_temp(index_temp,:)',peak_pos_sCSD_final_lmax12(:,2))<0)
        ...
        ang11 = atan2(norm(cross(fod_temp(1,:),peak_pos_sCSD_final_lmax12(:,3))), dot(fod_temp(1,:),peak_pos_sCSD_final_lmax12(:,3)));
        ang12 = abs(pi - atan2(norm(cross(fod_temp(1,:),peak_pos_sCSD_final_lmax12(:,3))), dot(fod_temp(1,:),peak_pos_sCSD_final_lmax12(:,3))));
        ang111 = min(ang11,ang12);
        angle3_sCSD_lmax12(rep,1) = ang111;
        if(id_temp(1) == 1)
            phi1_sCSD12(rep) = phi_temp;
            theta1_sCSD12(rep) = theta_temp;
        elseif(id_temp(1) == 2)
        ...
        sep1_sCSD_lmax12(rep,1) = min(ang11, ang12);
        
        ang11 = atan2(norm(cross(peak_pos_sCSD_final_lmax12(:,1),peak_pos_sCSD_final_lmax12(:,3))), dot(peak_pos_sCSD_final_lmax12(:,1),peak_pos_sCSD_final_lmax12(:,3)));
        ang12 = abs(pi - atan2(norm(cross(peak_pos_sCSD_final_lmax12(:,1),peak_pos_sCSD_final_lmax12(:,3))), dot(peak_pos_sCSD_final_lmax12(:,1),peak_pos_sCSD_final_lmax12(:,3))));
        sep2_sCSD_lmax12(rep,1) = min(ang11, ang12);
        
        ang11 = atan2(norm(cross(peak_pos_sCSD_final_lmax12(:,3),peak_pos_sCSD_final_lmax12(:,2))), dot(peak_pos_sCSD_final_lmax12(:,3),peak_pos_sCSD_final_lmax12(:,2)));
        ang12 = abs(pi - atan2(norm(cross(peak_pos_sCSD_final_lmax12(:,3),peak_pos_sCSD_final_lmax12(:,2))), dot(peak_pos_sCSD_final_lmax12(:,3),peak_pos_sCSD_final_lmax12(:,2))));
        sep3_sCSD_lmax12(rep,1) = min(ang11, ang12);
    end
   ...
   if(nfib_sCSD_lmax16(rep)==3)
        ...
        ang31 = atan2(norm(cross(fod3_s,peak_pos_sCSD_final_lmax16(:,1))), dot(fod3_s,peak_pos_sCSD_final_lmax16(:,1)));
        ang32 = abs(pi - atan2(norm(cross(fod3_s,peak_pos_sCSD_final_lmax16(:,1))), dot(fod3_s,peak_pos_sCSD_final_lmax16(:,1))));
        ang333 = min(ang31,ang32);
        
        [angle1_sCSD_lmax16(rep,1), index_temp] = min([ang111, ang222, ang333]);
        %%%%%
        ...
        elseif(index_temp == 2)
            ...
            phi3_sCSD16(rep) = phi_temp;
            theta3_sCSD16(rep) = theta_temp;
        end
        %%%%%
        ...
        ang21 = atan2(norm(cross(fod_temp(2,:),peak_pos_sCSD_final_lmax16(:,2))), dot(fod_temp(2,:),peak_pos_sCSD_final_lmax16(:,2)));
        ang22 = abs(pi - atan2(norm(cross(fod_temp(2,:),peak_pos_sCSD_final_lmax16(:,2))), dot(fod_temp(2,:),peak_pos_sCSD_final_lmax16(:,2))));
        ang222 = min(ang21,ang22);
        
        [angle2_sCSD_lmax16(rep,1), index_temp] = min([ang111, ang222]);
        
        %%%%%
        if(dot(fod_temp(index_temp,:)',peak_pos_sCSD_final_lmax16(:,2))<0)
        ...
        ang11 = atan2(norm(cross(fod_temp(1,:),peak_pos_sCSD_final_lmax16(:,3))), dot(fod_temp(1,:),peak_pos_sCSD_final_lmax16(:,3)));
        ang12 = abs(pi - atan2(norm(cross(fod_temp(1,:),peak_pos_sCSD_final_lmax16(:,3))), dot(fod_temp(1,:),peak_pos_sCSD_final_lmax16(:,3))));
        ang111 = min(ang11,ang12);
        angle3_sCSD_lmax16(rep,1) = ang111;
        if(id_temp(1) == 1)
            phi1_sCSD16(rep) = phi_temp;
            theta1_sCSD16(rep) = theta_temp;
        elseif(id_temp(1) == 2)
        ...
        sep1_sCSD_lmax16(rep,1) = min(ang11, ang12);
        
        ang11 = atan2(norm(cross(peak_pos_sCSD_final_lmax16(:,1),peak_pos_sCSD_final_lmax16(:,3))), dot(peak_pos_sCSD_final_lmax16(:,1),peak_pos_sCSD_final_lmax16(:,3)));
        ang12 = abs(pi - atan2(norm(cross(peak_pos_sCSD_final_lmax16(:,1),peak_pos_sCSD_final_lmax16(:,3))), dot(peak_pos_sCSD_final_lmax16(:,1),peak_pos_sCSD_final_lmax16(:,3))));
        sep2_sCSD_lmax16(rep,1) = min(ang11, ang12);
        
        ang11 = atan2(norm(cross(peak_pos_sCSD_final_lmax16(:,3),peak_pos_sCSD_final_lmax16(:,2))), dot(peak_pos_sCSD_final_lmax16(:,3),peak_pos_sCSD_final_lmax16(:,2)));
        ang12 = abs(pi - atan2(norm(cross(peak_pos_sCSD_final_lmax16(:,3),peak_pos_sCSD_final_lmax16(:,2))), dot(peak_pos_sCSD_final_lmax16(:,3),peak_pos_sCSD_final_lmax16(:,2))));
        sep3_sCSD_lmax16(rep,1) = min(ang11, ang12);
    end
    ...
    if(nfib_SN(rep)==3)
        ...
        ang31 = atan2(norm(cross(fod3_s,peak_pos_SN_final(:,1))), dot(fod3_s,peak_pos_SN_final(:,1)));
        ang32 = abs(pi - atan2(norm(cross(fod3_s,peak_pos_SN_final(:,1))), dot(fod3_s,peak_pos_SN_final(:,1))));
        ang333 = min(ang31,ang32);
        
        [angle1_SN(rep,1), index_temp] = min([ang111, ang222, ang333]);
        %%%%%
        ...
        elseif(index_temp == 2)
            ...
            phi3_SN(rep) = phi_temp;
            theta3_SN(rep) = theta_temp;
        end
        %%%%%
        ...
        ang21 = atan2(norm(cross(fod_temp(2,:),peak_pos_SN_final(:,2))), dot(fod_temp(2,:),peak_pos_SN_final(:,2)));
        ang22 = abs(pi - atan2(norm(cross(fod_temp(2,:),peak_pos_SN_final(:,2))), dot(fod_temp(2,:),peak_pos_SN_final(:,2))));
        ang222 = min(ang21,ang22);
        
        [angle2_SN(rep,1), index_temp] = min([ang111, ang222]);
        
        %%%%%
        if(dot(fod_temp(index_temp,:)',peak_pos_SN_final(:,2))<0)
        ...
        ang11 = atan2(norm(cross(fod_temp(1,:),peak_pos_SN_final(:,3))), dot(fod_temp(1,:),peak_pos_SN_final(:,3)));
        ang12 = abs(pi - atan2(norm(cross(fod_temp(1,:),peak_pos_SN_final(:,3))), dot(fod_temp(1,:),peak_pos_SN_final(:,3))));
        ang111 = min(ang11,ang12);
        angle3_SN(rep,1) = ang111;
        if(id_temp(1) == 1)
            phi1_SN(rep) = phi_temp;
            theta1_SN(rep) = theta_temp;
        elseif(id_temp(1) == 2)
        ...
        sep1_SN(rep,1) = min(ang11, ang12);
        
        ang11 = atan2(norm(cross(peak_pos_SN_final(:,1),peak_pos_SN_final(:,3))), dot(peak_pos_SN_final(:,1),peak_pos_SN_final(:,3)));
        ang12 = abs(pi - atan2(norm(cross(peak_pos_SN_final(:,1),peak_pos_SN_final(:,3))), dot(peak_pos_SN_final(:,1),peak_pos_SN_final(:,3))));
        sep2_SN(rep,1) = min(ang11, ang12);
        
        ang11 = atan2(norm(cross(peak_pos_SN_final(:,3),peak_pos_SN_final(:,2))), dot(peak_pos_SN_final(:,3),peak_pos_SN_final(:,2)));
        ang12 = abs(pi - atan2(norm(cross(peak_pos_SN_final(:,3),peak_pos_SN_final(:,2))), dot(peak_pos_SN_final(:,3),peak_pos_SN_final(:,2))));
        sep3_SN(rep,1) = min(ang11, ang12);
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%
rate_SH8 = sum(nfib_SH_lmax8==3)/N_rep;
under_SH8 = sum(nfib_SH_lmax8<3)/N_rep;
over_SH8 = sum(nfib_SH_lmax8>3)/N_rep;

rate_sCSD8 = sum(nfib_sCSD_lmax8==3)/N_rep;
under_sCSD8 = sum(nfib_sCSD_lmax8<3)/N_rep;
over_sCSD8 = sum(nfib_sCSD_lmax8>3)/N_rep;

rate_sCSD12 = sum(nfib_sCSD_lmax12==3)/N_rep;
under_sCSD12 = sum(nfib_sCSD_lmax12<3)/N_rep;
over_sCSD12 = sum(nfib_sCSD_lmax12>3)/N_rep;

rate_sCSD16 = sum(nfib_sCSD_lmax16==3)/N_rep;
under_sCSD16 = sum(nfib_sCSD_lmax16<3)/N_rep;
over_sCSD16 = sum(nfib_sCSD_lmax16>3)/N_rep;

rate_SN = sum(nfib_SN==3)/N_rep;
under_SN = sum(nfib_SN<3)/N_rep;
over_SN = sum(nfib_SN>3)/N_rep;
...
mean_angle1_SH_lmax8 = mean(angle1_SH_lmax8(nfib_SH_lmax8==3))*180/pi;
mean_angle1_sCSD_lmax8 = mean(angle1_sCSD_lmax8(nfib_sCSD_lmax8==3))*180/pi;
mean_angle1_sCSD_lmax12 = mean(angle1_sCSD_lmax12(nfib_sCSD_lmax12==3))*180/pi;
mean_angle1_sCSD_lmax16 = mean(angle1_sCSD_lmax16(nfib_sCSD_lmax16==3))*180/pi;
mean_angle1_SN = mean(angle1_SN(nfib_SN==3))*180/pi;

mean_angle2_SH_lmax8 = mean(angle2_SH_lmax8(nfib_SH_lmax8==3))*180/pi;
mean_angle2_sCSD_lmax8 = mean(angle2_sCSD_lmax8(nfib_sCSD_lmax8==3))*180/pi;
mean_angle2_sCSD_lmax12 = mean(angle2_sCSD_lmax12(nfib_sCSD_lmax12==3))*180/pi;
mean_angle2_sCSD_lmax16 = mean(angle2_sCSD_lmax16(nfib_sCSD_lmax16==3))*180/pi;
mean_angle2_SN = mean(angle2_SN(nfib_SN==3))*180/pi;

mean_angle3_SH_lmax8 = mean(angle3_SH_lmax8(nfib_SH_lmax8==3))*180/pi;
mean_angle3_sCSD_lmax8 = mean(angle3_sCSD_lmax8(nfib_sCSD_lmax8==3))*180/pi;
mean_angle3_sCSD_lmax12 = mean(angle3_sCSD_lmax12(nfib_sCSD_lmax12==3))*180/pi;
mean_angle3_sCSD_lmax16 = mean(angle3_sCSD_lmax16(nfib_sCSD_lmax16==3))*180/pi;
mean_angle3_SN = mean(angle3_SN(nfib_SN==3))*180/pi;


%%%%%
angle1_std_SH_lmax8 = std(angle1_SH_lmax8(nfib_SH_lmax8==3));
angle1_std_sCSD_lmax8 = std(angle1_sCSD_lmax8(nfib_sCSD_lmax8==3));
angle1_std_sCSD_lmax12 = std(angle1_sCSD_lmax12(nfib_sCSD_lmax12==3));
angle1_std_sCSD_lmax16 = std(angle1_sCSD_lmax16(nfib_sCSD_lmax16==3));
angle1_std_SN = std(angle1_SN(nfib_SN==3));

angle2_std_SH_lmax8 = std(angle2_SH_lmax8(nfib_SH_lmax8==3));
angle2_std_sCSD_lmax8 = std(angle2_sCSD_lmax8(nfib_sCSD_lmax8==3));
angle2_std_sCSD_lmax12 = std(angle2_sCSD_lmax12(nfib_sCSD_lmax12==3));
angle2_std_sCSD_lmax16 = std(angle2_sCSD_lmax16(nfib_sCSD_lmax16==3));
angle2_std_SN = std(angle2_SN(nfib_SN==3));

angle3_std_SH_lmax8 = std(angle3_SH_lmax8(nfib_SH_lmax8==3));
angle3_std_sCSD_lmax8 = std(angle3_sCSD_lmax8(nfib_sCSD_lmax8==3));
angle3_std_sCSD_lmax12 = std(angle3_sCSD_lmax12(nfib_sCSD_lmax12==3));
angle3_std_sCSD_lmax16 = std(angle3_sCSD_lmax16(nfib_sCSD_lmax16==3));
angle3_std_SN = std(angle3_SN(nfib_SN==3));


%%%%%

%%%%%
mean_sep1_SH_lmax8 = mean(sep1_SH_lmax8(nfib_SH_lmax8==3))*180/pi;
mean_sep1_sCSD_lmax8 = mean(sep1_sCSD_lmax8(nfib_sCSD_lmax8==3))*180/pi;
mean_sep1_sCSD_lmax12 = mean(sep1_sCSD_lmax12(nfib_sCSD_lmax12==3))*180/pi;
mean_sep1_sCSD_lmax16 = mean(sep1_sCSD_lmax16(nfib_sCSD_lmax16==3))*180/pi;
mean_sep1_SN = mean(sep1_SN(nfib_SN==3))*180/pi;

sd_sep1_SH_lmax8 = std(sep1_SH_lmax8(nfib_SH_lmax8==3))*180/pi;
sd_sep1_sCSD_lmax8 = std(sep1_sCSD_lmax8(nfib_sCSD_lmax8==3))*180/pi;
sd_sep1_sCSD_lmax12 = std(sep1_sCSD_lmax12(nfib_sCSD_lmax12==3))*180/pi;
sd_sep1_sCSD_lmax16 = std(sep1_sCSD_lmax16(nfib_sCSD_lmax16==3))*180/pi;
sd_sep1_SN = std(sep1_SN(nfib_SN==3))*180/pi;

mean_sep2_SH_lmax8 = mean(sep2_SH_lmax8(nfib_SH_lmax8==3))*180/pi;
mean_sep2_sCSD_lmax8 = mean(sep2_sCSD_lmax8(nfib_sCSD_lmax8==3))*180/pi;
mean_sep2_sCSD_lmax12 = mean(sep2_sCSD_lmax12(nfib_sCSD_lmax12==3))*180/pi;
mean_sep2_sCSD_lmax16 = mean(sep2_sCSD_lmax16(nfib_sCSD_lmax16==3))*180/pi;
mean_sep2_SN = mean(sep2_SN(nfib_SN==3))*180/pi;

sd_sep2_SH_lmax8 = std(sep2_SH_lmax8(nfib_SH_lmax8==3))*180/pi;
sd_sep2_sCSD_lmax8 = std(sep2_sCSD_lmax8(nfib_sCSD_lmax8==3))*180/pi;
sd_sep2_sCSD_lmax12 = std(sep2_sCSD_lmax12(nfib_sCSD_lmax12==3))*180/pi;
sd_sep2_sCSD_lmax16 = std(sep2_sCSD_lmax16(nfib_sCSD_lmax16==3))*180/pi;
sd_sep2_SN = std(sep2_SN(nfib_SN==3))*180/pi;

mean_sep3_SH_lmax8 = mean(sep3_SH_lmax8(nfib_SH_lmax8==3))*180/pi;
mean_sep3_sCSD_lmax8 = mean(sep3_sCSD_lmax8(nfib_sCSD_lmax8==3))*180/pi;
mean_sep3_sCSD_lmax12 = mean(sep3_sCSD_lmax12(nfib_sCSD_lmax12==3))*180/pi;
mean_sep3_sCSD_lmax16 = mean(sep3_sCSD_lmax16(nfib_sCSD_lmax16==3))*180/pi;
mean_sep3_SN = mean(sep3_SN(nfib_SN==3))*180/pi;


sd_sep3_SH_lmax8 = std(sep3_SH_lmax8(nfib_SH_lmax8==3))*180/pi;
sd_sep3_sCSD_lmax8 = std(sep3_sCSD_lmax8(nfib_sCSD_lmax8==3))*180/pi;
sd_sep3_sCSD_lmax12 = std(sep3_sCSD_lmax12(nfib_sCSD_lmax12==3))*180/pi;
sd_sep3_sCSD_lmax16 = std(sep3_sCSD_lmax16(nfib_sCSD_lmax16==3))*180/pi;
sd_sep3_SN = std(sep3_SN(nfib_SN==3))*180/pi;
...
savefig(strcat(save_path,'3fib_sep',num2str(sep*180/pi),'_lmax',num2str(lmax),'_b',num2str(b(1)),'_ratio',num2str(ratio(1)),'_N',num2str(n_sample),'.fig'));
...
%%%%% latex table
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
SH_input = round([rate_SH8 under_SH8 over_SH8 mean_angle1_SH_lmax8 mean_angle2_SH_lmax8 mean_angle3_SH_lmax8...
    mean_sep1_SH_lmax8 mean_sep2_SH_lmax8 mean_sep3_SH_lmax8 mean_HD_SH_lmax8 sd_HD_SH_lmax8],4);
sCSD8_input = round([rate_sCSD8 under_sCSD8 over_sCSD8 mean_angle1_sCSD_lmax8 mean_angle2_sCSD_lmax8 mean_angle3_sCSD_lmax8...
    mean_sep1_sCSD_lmax8 mean_sep2_sCSD_lmax8 mean_sep3_sCSD_lmax8 mean_HD_sCSD_lmax8 sd_HD_sCSD_lmax8],4);
sCSD12_input = round([rate_sCSD12 under_sCSD12 over_sCSD12 mean_angle1_sCSD_lmax12 mean_angle2_sCSD_lmax12 mean_angle3_sCSD_lmax12...
    mean_sep1_sCSD_lmax12 mean_sep2_sCSD_lmax12 mean_sep3_sCSD_lmax12 mean_HD_sCSD_lmax12 sd_HD_sCSD_lmax12],4);
sCSD16_input = round([rate_sCSD16 under_sCSD16 over_sCSD16 mean_angle1_sCSD_lmax16 mean_angle2_sCSD_lmax16 mean_angle3_sCSD_lmax16... 
    mean_sep1_sCSD_lmax16 mean_sep2_sCSD_lmax16 mean_sep3_sCSD_lmax16 mean_HD_sCSD_lmax16 sd_HD_sCSD_lmax16],4);
SN_input = round([rate_SN under_SN over_SN mean_angle1_SN mean_angle2_SN mean_angle3_SN...
    mean_sep1_SN mean_sep2_SN mean_sep3_SN mean_HD_SN sd_HD_SN],4);

input.data = [SH_input;sCSD8_input;sCSD12_input;SN_input];

input.tableColLabels = {'Correct','Under','Over', 'Error-1', 'Error-2', 'Error-3','Mean Sep1.', 'Mean Sep2.', 'Mean Sep3.','Mean H-dist.', 'SD H-dist.'};
...
input.dataFormat = {'%.2f',10,'%.4f',1}; % three digits precision for first two columns, one digit for the last
