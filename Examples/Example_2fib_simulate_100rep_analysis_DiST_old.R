...
for(rep in 1:N_rep){
  ...
  if(out$n.fiber == 2){
    fib_temp = out$vs
    # theta.r = temp$theta0 - temp$theta0s #(temp$theta0s)  #temp$theta.r
    # phi.r = temp$phi0 - temp$phi0s #(temp$phi0s)  #temp$phi0
    # theta.r = -temp$theta0 + temp$theta0s #(temp$theta0s)  #temp$theta.r
    # phi.r = -temp$phi0 + temp$phi0s #(temp$phi0s)  #temp$phi0
    # c(sin(temp$theta0[1])*cos(temp$phi0[1]),sin(temp$theta0[1])*sin(temp$phi0[1]),cos(temp$theta0[1]))
    ...
    fib_temp = out$vs 
    # theta.r = temp$theta0 - t(temp$theta.r) #(temp$theta0s)  #temp$theta.r
    # phi.r = temp$phi0 - t(temp$phi.r) #(temp$phi0s)  #temp$phi0
    # rotationM1 = matrix(c(cos(phi.r[1])*cos(theta.r[1]),-sin(phi.r[1]),
    #                       cos(phi.r[1])*sin(theta.r[1]),sin(phi.r[1])*cos(theta.r[1]),
    #                       cos(phi.r[1]),sin(phi.r[1])*sin(theta.r[1]),-sin(theta.r[1]),
    #                       0,cos(theta.r[1])),nrow=3)
    ...
    fib_true_DiST[rep,1,] = temp$fod1  #temp$fod1.s%*%rotationM1
    fib_true_DiST[rep,2,] = temp$fod2  #temp$fod2.s%*%rotationM2
