...
path_folder = strcat('/Users/hao/Dropbox/stats_project/FOD_codes_simulation/Real_data/', subject_id, '/diff/preproc/','nii_b',num2str(b/1000),'/');
...
%% load nii data (raw)
bvec_raw = load(strcat(path_folder,'bvecs.txt')); 
bval_raw = load(strcat(path_folder,'bvals.txt'));
nii_data = load_untouch_nii(strcat(path_folder,'nii_data_b',num2str(b/1000),'.nii.gz'));
% nii_data_header = nii_data.hdr;

%% load fsl processed data
nii_FA = load_untouch_nii('dti_FA.nii.gz');
nii_MO = load_untouch_nii('dti_MO.nii.gz');
nii_MD = load_untouch_nii('dti_MD.nii.gz');
nii_S0 = load_untouch_nii('dti_S0.nii.gz');

nii_V1 = load_untouch_nii('dti_V1.nii.gz');
nii_V2 = load_untouch_nii('dti_V2.nii.gz');
nii_V3 = load_untouch_nii('dti_V3.nii.gz');

nii_L1 = load_untouch_nii('dti_L1.nii.gz');
nii_L2 = load_untouch_nii('dti_L2.nii.gz');
nii_L3 = load_untouch_nii('dti_L3.nii.gz');
...
path_save = strcat(path_folder,'fitting_',voxel_sub,'/');
mkdir(path_save);
...
title('SNR')
savefig(strcat(path_save,'PreAnalysis_',voxel_sub,'.fig'))
...
%% single tensor model
for j=1:length(index_temp)       % N   %length(index_temp)
    i = index_temp(j);
    l_S = log(S_data(:,i));
...
savefig(strcat(path_save,'PreAnalysis_singleTensor',voxel_sub,'.fig'))
...
%         Eig1_fig = figure;
        ...
        display(i3);
        savefig(strcat(path_save,'FA_MD_color_slice',num2str(i3),'.fig'))
    end
    options.use_axis = 0;
%}
...
%                     alpha(0.25)
            ...
            hold off;
            savefig(strcat(path_save,'SN_FA_est_z_','slice', num2str(i3),'.fig'));          
            display(i3);
